...
router.get('/', async (req, res) => {
  try {
    ...
    console.error('[GET /monitors]', err.message);
...
router.post('/', async (req, res) => {
  try {
    ...
    console.error('[POST /monitors]', err.message);
    res.status(500).json({ error: err.message });
  }
});
...
router.put('/:id', async (req, res) => {
  try {
    ...
    console.error('[PUT /monitors/:id]', err.message);
    res.status(500).json({ error: err.message });
  }
});
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    console.error('[DELETE /monitors/:id]', err.message);
    res.status(500).json({ error: err.message });
  }
});
...
router.post('/:id/check', async (req, res) => {
  try {
    ...
    console.error('[POST /monitors/:id/check]', err.message);
    res.status(500).json({ error: err.message });
  }
});
